	...
	<Listbox v-model="selectedItem">
		<div class="relative" :class="props.className">
			...
			<transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
				...
				<ListboxOptions style="z-index: 100"
					class="absolute w-full py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
					<ListboxOption v-slot="{ active, selected }" v-for="item in sourceArr" :key="`${item.id}`" :value="item"
						as="template">
						<li :class="[
							...
							<span v-if="selected" class="absolute inset-y-0 left-0 flex items-center pl-3 text-amber-500">
...
// 如果没有找到匹配项（index === -1），使用第一个选项作为默认值
// 如果 sourceArr 为空，返回一个默认对象避免 undefined 错误
const selectedItem = ref(
	props.sourceArr.length > 0
		? props.sourceArr[index !== -1 ? index : 0]
		: { id: '', name: '' }
)
