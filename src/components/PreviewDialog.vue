				...
				<div class="flex items-center justify-center min-h-full p-4 text-center">
					<TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
						...
						<DialogPanel
							...
							<div class="flex items-center justify-start flex-auto px-2 py-2 m-4 rounded-md shadow-inner bg-slate-50">
								<div class="flex flex-col flex-wrap items-center justify-center h-20 mx-2">
									<p class="pb-2 font-medium text-gray-400">FILTERS</p>
									<HeadlessSelect className="w-36" :sourceArr="FILTERS_ARR" :defaultId="FILTERS_ARR[0].id"
										@selected="handleFilter" />
								</div>
								<button class="block h-12 p-2 m-2 border border-gray-300 rounded-md w-36 hover:bg-gray-50"
...
const filterImage = async (style: String) => {
	const img = await generateImg() as HTMLImageElement
...
function renderNewImage(img: HTMLImageElement, size: any) {
	// Create a canvas and get a 2D context from the canvas
	const canvas: any = document.getElementById('preview-area')
